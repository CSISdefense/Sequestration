...
on ccid.CSIScontractID =contracttotal.CSIScontractID
	...
	insert into Contract.UnlabeledDunsnumberUniqueTransactionIDentityIDhistory
...
,StandardizedVendorName)
	select c.unique_transaction_id
	,c.fiscal_year
	,vn.vendorname 
	...
	where 	(Parent.ParentID is not null or isnull(Parent.UnknownCompany,0)=1)
...
on vn.EntityID=ent.EntityID
where vn.Vendorname=ent.VendorName
and vn.entityid is null


--	update c
--	set StandardizedVendorName=coalesce(vn.StandardizedVendorName
--			,vlon.StandardizedVendorName
--			,vdabn.StandardizedVendorName
--			,van.StandardizedVendorName
--			,dn.StandardizedVendorName
...
--	from Contract.UnlabeledDunsnumberUniqueTransactionIDentityIDhistory c
...
--	select distinct ParentID,FiscalYear
--into Vendor.ParentIDHistory
--from contractor.DunsnumberToParentContractorHistory
--order by FiscalYear,ParentID


--CREATE TABLE Vendor.EntitySizeCode(
--	EntitySizeCode varchar(1) primary key,
--	EntitySizeText varchar(15)
--)

--alter table Vendor.EntitySizeCode
--alter column EntitySizeText varchar(25)

--insert into Vendor.EntitySizeCode
--(EntitySizeCode,EntitySizeText)
--values ('J','Large: Big 5 JV')

--alter table Contract.UnlabeledDunsnumberUniqueTransactionIDentityIDhistory
--add IsEntityAbove1990constantReportingThreshold bit
--,EntitySizeCode varchar(1) references Vendor.EntitySizeCode(EntitySizeCode)
--,IsEntityAbove2016constantReportingThreshold bit
--,AnyEntityUSplaceOfPerformance bit
----add EntitySizeCode varchar(1) references Vendor.EntitySizeCode(EntitySizeCode)
...
--alter table Contractor.ParentContractor
--add EntityID int references Vendor.EntityID(EntityID)

--alter table Contractor.Dunsnumber
--add EntityID int references Vendor.EntityID(EntityID)

--alter table Vendor.Vendorname
--add EntityID int references Vendor.EntityID(EntityID)
...
--select country.isforeign
--,~sc.usa as NotUSA
--,country.[alpha-3]
--,c3lc.IsInternational
...
--		left outer join FPDSTypeTable.statecode sc
--		on sc.statecode=c.pop_state_code 
--		where cast(coalesce(country.isforeign, ~ sc.usa) as tinyint) is null
--	GROUP BY country.isforeign
--,~sc.usa 
--,country.[alpha-3]
--,c3lc.IsInternational
--,c.PlaceofPerformanceCity
