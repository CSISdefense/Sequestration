  ...
  mainplot <- reactive({
    ...
    if(input$chart_geom == "Period Stacked"){
      ...
      line_plot <- build_plot(data=share_data,
                              ...
                              facet_var=input$facet_var,
                              labels_and_colors=labels_and_colors,
                              column_key=column_key)+labs(
          y="Share Contract $", caption = "Source: FPDS; CSIS analysis"
        ) + theme(plot.caption = element_text(size=7, face = "italic", vjust= -0.15, color = "gray25")
                  
        )
      line_plot<-line_plot+theme(legend.position = "none")
      ...
      mainplot <- build_plot(data=plot_data,
                             ...
                             labels_and_colors=labels_and_colors,
                             column_key=column_key)+labs(
           caption = "Source: FPDS; CSIS analysis") + theme(plot.caption = element_text(size=7, face = "italic", vjust= -0.15, color = "gray25")
        )
